...
import { getTrustWalletChainName, getDebankChainName } from "@/lib/config"
...
export function getTokenLogoUrl(
  ...
  // Fallback to DeBank
  const debankChain = getDebankChainName(chainId)
  if (debankChain && token.contract_address && isAddress(token.contract_address)) {
    return `https://static.debank.com/image/${debankChain}_token/logo_url/${token.contract_address.toLowerCase()}/logo_url.png`
  }
